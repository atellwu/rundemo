(function(a, c) {
	var b = function(e, d) {
		...
		this.render();
		//增加代码--预加载，为了根据链接定位代码文件所在文件树的位置
		if(d.headId!=null){
			var headIds = d.headId.split("_");
			var temp;
			for(var i = 0;i<headIds.length-1;i++){
				if(i == 0){
					temp = headIds[i];
				}else{
					temp = temp + "_" + headIds[i];
				}
				this.selectFolder($('#h_'+temp).parent().parent().get(0));
			}
		}
