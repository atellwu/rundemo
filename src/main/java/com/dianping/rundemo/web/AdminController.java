...
public class AdminController {
   ...
   public RedirectView create(String app, String gitUrl, String subpath, HttpServletRequest request,
                              HttpServletResponse response) throws FileNotFoundException, IOException {
      //调用shell，下载git到本地，并且求出classpath
      InputStream input = null;
      try {
         Process proc = Runtime.getRuntime().exec(new String[] { "/data/rundemo/down_git.sh", gitUrl, app });
         input = proc.getInputStream();
         LOG.info("down_git.sh output:" + IOUtils.LINE_SEPARATOR + IOUtils.toString(input));
      } finally {
